...
describe('NecordParamsFactory', () => {
	...
	it('should map CONTEXT to args[0]', () => {
		const res = factory.exchangeKeyForValue(NecordParamType.CONTEXT, undefined, [
			context,
			discovery
		]);
	...
	it('should map DISCOVERY to args[1]', () => {
		const res = factory.exchangeKeyForValue(NecordParamType.DISCOVERY, undefined, [
			context,
			discovery
		]);
	...
	it('should return null for unknown param type', () => {
		const res = factory.exchangeKeyForValue(999 as any, undefined, [context, discovery]);
		expect(res).toBeNull();
	});
